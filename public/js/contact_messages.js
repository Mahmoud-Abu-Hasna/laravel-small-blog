/**
 * Created by mabuhasna on 6/30/2017.
 */
...
function modalContent(event) {
  ...
  //  var sender = event.path[5].lastElementChild.firstElementChild.innerText;
    var sender = event.path[5].firstElementChild.firstElementChild.nextElementSibling.innerText;
...
var article = params[0];
    if(success){
        article.style.backgroundColor='#ffc4bc';
        setTimeout(function(){
            article.remove();
            location.reload();
        },300);
